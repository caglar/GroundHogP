...
def jobman(state, channel):
    ...
    mlp_out = x_train
    for i in xrange(state['nlayers']):
        mlp_out = ff_layers[i].fprop(mlp_out)
    ...
    for i in xrange(state['nlayers']):
        mlp_valid_out = ff_layers[i].fprop(mlp_valid_out,
                                           use_noise=False)


    valid_model = output_layer(mlp_valid_out,
                               use_noise=False).validate(target=y, hints=hints)

    valid_fn = theano.function([x_valid, y, hints],
                               [valid_model.cost,
                                valid_model.out[0],
                                valid_model.out[1]],
                                name = 'valid_fn',
