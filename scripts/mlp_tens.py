...
def jobman(state, channel):
    ...
    train_model = output_sto.train(target=y)\
            / TT.cast(y.shape[0], 'float32')
    ...
    valid_model = output_layer(mlp_valid_out,
                               target=y_valid,
                               use_noise=False).validate(target=y_valid)

    valid_fn = theano.function([x_valid, y_valid],
                               [valid_model.cost,
                                valid_model.out[0]],
                                name = 'valid_fn',
