...
from groundhog.datasets.mnist_dataset import MnistIterator
...
def get_data(state):
    ...
    path=None
    train_data = MnistIterator(batch_size = int(state['bs']),
                               mode='train',
                               path=path,
                               output_format=out_format)

    valid_data = MnistIterator(batch_size = int(state['bs']),
                               mode='valid',
                               path=path,
                               use_infinite_loop = False,
                               output_format=out_format)
    test_data = MnistIterator(batch_size = int(state['bs']),
                              mode='test',
                              path=path, use_infinite_loop = False,
                              output_format=out_format)
