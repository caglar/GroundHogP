...
class SGD(object):
    ...
    def __init__(self,
        ...
        if 'adagrad' not in state:
            self.adagrad = False
        else:
            self.adagrad = state['adagrad']

        f32 = np.float32
        eps = f32(state['adaeps'])
        ...
        self.old_delta_gs = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape,
                                             dtype=theano.config.floatX) + numpy.float32(eps),
                                name=p.name+'_old_delta_gs')
                    for p in model.params]
        ...
        self.sum_sqr_gs = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape,
                                                     dtype=theano.config.floatX) + numpy.float32(eps),
                                         name = p.name + '_sum_sqr_gs')
                           for p in model.params]
        ...
        if self.adagrad:
            new_sum_sqr_lst = [ ssg + TT.sqr(g) for ssg, g in zip(self.sum_sqr_gs, normed_gs) ]
        ...
        for (delta_sqr, g, curve, old_g, ave_g,
             gamm_n, gamma_d, ave_delta, tau, og,
                               old_delta_gs, nsg) in zip_its:
            ...
            old_gs_up.append((og, g))#corrected_grad))
            steplen = TT.sqrt(delta_sqr + f32(eps)) / TT.sqrt(curve + f32(eps))

            if self.adagrad:
                rms_g_t = TT.maximum(TT.sqrt(nsg), 1.0)
                steplen = steplen / rms_g_t

            steplen = TT.cast(steplen.clip(-8, 8), theano.config.floatX)
            ...
            old_delta_gs_lst.append(delta_x_t)

            steps.append(delta_x_t)

            if bs < 2500:
                delta_x_t_sqr = TT.sqr(delta_x_t - old_delta_gs)
            else:
                delta_x_t_sqr = TT.sqr(delta_x_t)

            new_delta_sqr_gs_lst.append(delta_sqr * (f32(1) - (f32(1) / tau)) + (f32(1) / tau) * delta_x_t_sqr)
            ave_delta_lst.append(ave_delta * (f32(1) - (f32(1) / tau)) + (f32(1) / tau) * delta_x_t)
        ...
        old_delta_gs_up = zip(self.old_delta_gs, old_delta_gs_lst)
