...
class SGD(object):
    def __init__(self,
        ...
        self.dnorm2 = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape,
                                ...
                                name=p.name+'_d2')
                   for p in model.params]
        self.agnorm2 = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape,
                                             dtype=theano.config.floatX),
                                name=p.name+'_ag2')
                   for p in model.params]

        self.adnorm2 = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape,
                                             dtype=theano.config.floatX),
                                name=p.name+'_ad2')
                   for p in model.params]
	...
	if 'profile' not in self.state:
        ...
        agnorm2_up = [rho * agn2 + (1. - rho) * g for agn2,g in zip(self.agnorm2, gs)]

        updates = updates + zip(self.gnorm2, gnorm2_up)
        updates = updates + zip(self.agnorm2, agnorm2_up)
        ...
        new_params = []
        for p, g, gn2, agn2, dn2, adn2 in zip(model.params, self.gs, self.gnorm2, self.agnorm2,
                                             self.dnorm2, self.adnorm2):
            if self.mom > 0:
                new_params_up = (p,
                    p - (TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2 - agn2**2 + eps))* (g + self.mom * agn2))
            else:
                new_params_up = (p,
                    p - (TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2 - agn2**2 + eps))*g)
        ...
        for p, g, gn2, agn2, dn2, adn2 in zip(model.params, self.gs, self.gnorm2, self.agnorm2,
                                              self.dnorm2, self.adnorm2):
            if self.mom > 0:
                d2_up = (dn2, rho * dn2 + (1 - rho)* (((TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2
                    - agn2**2 + eps)*(g + self.mom*agn2)))**2))
                ad2_up = (adn2, rho*adn2 + (1-rho)*((TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2
                    - agn2**2 + eps)*(g + self.mom*agn2))))
            else:
                d2_up = (dn2, rho * dn2 + (1 - rho)* (((TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2
                    - agn2**2 + eps)*g))**2))
                ad2_up = (adn2, rho*adn2 + (1-rho)*((TT.sqrt(dn2 - adn2**2 + eps) / TT.sqrt(gn2
                    - agn2**2 + eps)*g)))
            d2_ups.append(d2_up)
            ad2_ups.append(ad2_up)

        updates = updates + d2_ups
        updates = updates + ad2_ups
