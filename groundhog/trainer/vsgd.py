...
class SGD(object):
    def __init__(self,
        ...
        self.model = model
        eps = f32(1e-8)
        self.epsilon = theano.shared(f32(2.1), name="epsilon")
        ...
        self.mean_gs = [theano.shared(numpy.ones(p.get_value(borrow=True).shape,
                                             dtype=theano.config.floatX)*1e-7,
                                name=p.name+'_mg')
                   for p in model.params]
        ...
        self.time = [theano.shared(numpy.ones(p.get_value(borrow=True).shape,
                                             dtype=theano.config.floatX)*2 + 1e-8,
                                name=p.name+'_t')
                   for p in model.params]
        ...
        norm_gs = TT.sqrt(sum(TT.sum(x**2)
                          for x, p in zip(gs, self.model.params) if p not in self.model.params))
        ...
        new_params = [p + eps for p, mg in zip(model.params, self.mean_gs)]

        ngs = theano.clone(model.param_grads,
                           replace=zip(model.inputs, loc_data) + zip(model.params, new_params),
                           share_inputs=False)

        #ngs = [theano.printing.Print("ngs ")(ng) for ng in ngs]
        #mean_gs = [theano.printing.Print("mean gs ")(mg) for mg in self.mean_gs]

        hs = [abs((g - ng) / (eps)) for g, ng, mg in zip(gs,
                                                        ngs,
                                                        self.mean_gs)]

        #hs = [theano.printing.Print("hs ")(h) for h in hs]
        new_time = [TT.switch( TT.or_(abs(g - mg) > 2 * TT.sqrt((mvg - mg**2) / (bs - 1)),
                                      abs(h - mh) > 2 * TT.sqrt((mvh - mh**2) / (bs - 1))),
                                      TT.maximum(t+1, 1.3), TT.maximum(t, 1.3)) for g, mg, h, mh, mvg,
                                                                                    mvh, t in zip(self.gs,
                                                                                                  self.mean_gs, hs,
                                                                                                  self.mean_hs, self.mean_vsg,
                                                                                                  self.mean_vsh, self.time)]

        #new_time = [ theano.printing.Print("time ")(t) for t in new_time]

        new_mean_gs = [(1 - f32(1.) /(t)) * mg + (1. / t) *  g + eps for t, mg, g in zip(new_time,
                                                                                   self.mean_gs,
                                                                                   gs)]

        new_mean_vsg = [
            (1 - 1. / (t))* mv + (1. / t) * g**2 + eps for t, mv, g in zip(new_time,
                                                                     self.mean_vsg,
                                                                     gs)]

        new_mean_hs = [
            (1 - 1. / (t)) * mh + (1. / t) *  h + eps for t, mh, h in zip(new_time,
                                                                    self.mean_hs,
                                                                    hs)]

        new_mean_vsh = [
            (1 - 1. / (t))* mv + (1. / t) * h**2 + eps for t, mv, h in zip(new_time,
                                                                     self.mean_vsh,
                                                                     hs)]

        new_time = [
            (1 - mg**2 / (mv + 1e-8)) * t + 1 + eps for mg, mv, t in zip(new_mean_gs,
                                                        new_mean_vsg,
                                                        new_time)]
        ...
        lr = TT.scalar('lr')
        cur_step = theano.shared(f32(0), name="cur_step")

        self.lr = numpy.float32(state['lr'])
        new_params = [p - lr * g for p, g in zip(model.params, self.gs)]
        self.update_fn = theano.function([lr], [lr, lr, lr], name='update_function',
                                         allow_input_downcast=True,
                                         updates = zip(model.params, new_params) + new_updates,
                                         profile=0)

        etas = [(((mh + f32(1e-8)) / (mvh + f32(1e-8))) * ((f32(bs) * mg**2 + eps)/ (mvg + (f32(bs) - f32(1)) * mg**2 + f32(1e-8))))
                for mh, mvh, mg, mvg in zip(self.mean_hs, self.mean_vsh,
                                            self.mean_gs, self.mean_vsg)]

        new_steps = [(cur_step, cur_step + 1)]
        mean_eta = etas[0].sum()
        nelems = etas[0].shape.prod()
        max_eta = etas[0].max()
        min_eta = etas[0].min()

        for eta in etas[1:]:
            mean_eta += eta.sum()
            nelems += eta.shape.prod()
            max_eta = TT.maximum(eta.max(), max_eta)
            min_eta = TT.minimum(eta.min(), min_eta)

        mean_eta = mean_eta / nelems
        #etas = [ theano.printing.Print("eta ")(eta) for eta in etas]


        new_params = [p - TT.clip(TT.clip(eta, -4, 4) * g, -6, 6)
                          for p, g, eta in zip(model.params, gs, etas)]

        self.update_fn_new = theano.function([],
                                             [mean_eta, max_eta, min_eta],
    ...
    def __call__(self):
        ...
        rvals = self.train_fn()
        if self.step < self.state['switch']: #or self.step % self.state['switch'] == 0:
            lr_rval = self.update_fn(self.lr)
        else:
            lr_rval = self.update_fn_new()
