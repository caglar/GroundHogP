...
class SGD(object):
    def __init__(self,
        ...
        self.nstep = theano.shared(f32(0), name="nstep")
        ...
        new_cost_scale = theano.shared(f32(cost_scale.get_value()),
                                       name="cost_scale")
        ...
        tau = self.state['cost_tau']
        min_scale = self.state["cost_scale_min"]

        new_cost_scale_up = [(self.cost_scale, TT.switch(self.nstep > self.state['start_step'], TT.maximum(min_scale,
            self.state["cost_scales"][1] / tau**self.nstep), self.cost_scale))]
        new_step_up = [(self.nstep, self.nstep + 1)]

        # grad2
        gnorm2_up = [rho * gn2 + (1. - rho) * (g ** 2.) for gn2,g in zip(self.gnorm2, gs)]
        updates = updates + zip(self.gnorm2, gnorm2_up) + new_cost_scale_up + new_step_up
    ...
    def __call__(self):
        ...
        if self.step % self.state['trainFreq'] == 0:
            #print "Step is, ", self.nstep.get_value()
            msg = '.. iter %4d cost %.3f'
            vals = [self.step, cost]
            ...
            msg += ' cost_scale: %f ' % self.cost_scale.get_value()
